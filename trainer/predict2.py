...
def read_input(batch_size,filenames):
    ...
    inp = [np.asarray(batch_list1), np.asarray(batch_list2), np.asarray(batch_list3), np.asarray(batch_list4), np.asarray(batch_list5), low_res_black, high_res_black]
    return inp
...
if __name__ == '__main__':
    ...
    superes_net = load_model('model_old.h5', custom_objects={"tf": tf,
    ...
    for i in range(len(pred)):
        for j in range (len(pred[i])):
            output_path = r"C:\Users\Francis\Desktop\project_document\SuperRezil-master\trainer\output2\{j_}_{i_}.png".format(i_=i,j_ = j)
