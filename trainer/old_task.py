...
def generator2(filenames):
    ...
    while True:
        ...
        result, l = [np.asarray(batch_list1), np.asarray(batch_list2), np.asarray(batch_list3), np.asarray(batch_list4),
                     np.asarray(batch_list5), low_res_black, high_res_black, flow_pre], [
                        np.asarray(label_list1), np.asarray(label_list2), np.asarray(label_list3),
                        np.asarray(label_list4), np.asarray(label_list5)]
        yield result, l
...
if __name__ == '__main__':
    ...
    superes_net.model.fit_generator(generator(filenames), epochs=8, samples_per_epoch=21000,
                                    callbacks=[tensorboard, estop])

    superes_net.model.save("model_old.h5")
